...
namespace ATM
{
    public partial class AdminNearbyATMs : Form
    {
        ...
        {
            DGVPrinter printer = CreatePrinter("Nearby ATMs - Admin");
            printer.PrintDataGridView(dataGridView1);
            MessageBox.Show("Successfully printed.", "Admin - Nearby ATMs",
        ...
        private void LoadData(string prefix = "")
        {
            try
            {
                dataGridView1.DataSource = GridData.LoadNearbyAtms(prefix);
            }
            catch (Exception ex)
            {
                MessageBox.Show("ATM locations could not be loaded. " + ex.Message,
                    "Admin - Nearby ATMs", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
